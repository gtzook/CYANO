...
def led_loop(shared_data: Dict[str, Union[int,float,bool]], 
    ...
    pixels = neopixel.NeoPixel(
        board.D18, 103, brightness=.5, auto_write=False, pixel_order=ORDER
    )
    #Initialize to White
    ...
    while True:
        ...
        if pixels.brightness != shared_data['brightness']:
            if debug_mode:
                print(f"light_controller: brightness change request received to \
                      {shared_data['brightness']} from {pixels.brightness}")
